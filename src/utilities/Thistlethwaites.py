...
class ThistlethwaiteSolver:
    def __init__(self, cube_data=None):
        # Initialize RubiksCube instance from input or empty state
        if isinstance(cube_data, RubiksCube):
            self.cube = cube_data
        else:
            self.cube = RubiksCube()
            # Load cube faces if provided as dict
    ...
    def get_phase1_state(self):
        h = self.get_state_hash(1)
        # hash(1) = 8 corner‐twist digits + 4 E‑slice bits
        # first 8 must be '0', next 4 must be '0'
    ...
    def get_state_hash(self, phase):
        ...
        if phase == 0:
            ...
            # Only 4 E‑slice edges: 0 if in middle, 1 otherwise
            e_slice_colors = ['G','B','O','R']
            e_edges = [
                ('F',(1,0),'L',(1,2)),  # FL
                ('F',(1,2),'R',(1,0)),  # FR
            ...
            for f1,p1,f2,p2 in e_edges:
                s1 = self.cube.faces[f1][p1[0]][p1[1]]
                s2 = self.cube.faces[f2][p2[0]][p2[1]]
                bits.append('0' if (s1 in e_slice_colors and s2 in e_slice_colors) else '1')
            return ''.join(bits)
...
def solve_cube():
    try:
        ...
        cube = RubiksCube()
        cube.load_from_cube_data(cube_data)
        ...
        all_moves = solution1 + solution2 + solution3 + solution4
        moves_str = ' '.join(all_moves)
        print(all_moves)
        response1 = jsonify({'status': 'success', 'solution': moves_str})
